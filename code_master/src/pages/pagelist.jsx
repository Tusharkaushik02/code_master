...
function Problemlist() {
  ...
  const diffColor = (d) => d === 'Easy' ? '#00b8a3' : d === 'Medium' ? '#ffc01e' : '#ff375f';
  ...
  return (
    <div className="min-h-screen" style={{ background: '#1a1a2e', paddingTop: '70px' }}>
          ...
          <div className="sticky top-24 space-y-1">
            {sidebarItems.map((item, i) => (
              <button key={i} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium cursor-pointer transition-all duration-200"
                style={{ color: '#94a3b8', background: 'transparent', border: 'none', textAlign: 'left' }}
                ...
                {item.label}
                {item.badge && <span className="ml-auto text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(0,184,163,0.15)', color: '#00b8a3' }}>{item.badge}</span>}
              </button>
            ))}
            <div style={{ height: '1px', background: 'rgba(255,255,255,0.06)', margin: '12px 0' }} />
            <div className="px-4 flex items-center justify-between">
              <span className="text-sm font-medium" style={{ color: '#94a3b8' }}>My Lists</span>
              <button className="text-lg cursor-pointer" style={{ color: '#64748b', background: 'none', border: 'none' }}>+</button>
            </div>
        ...
        <main className="flex-1 min-w-0">
                ...
                return (
                  <button key={tag} onClick={() => setSelectedTag(tag)}
                    ...
                    style={{
                      ...
                      color: selectedTag === tag ? '#1a1a2e' : '#94a3b8',
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            {difficulties.map(d => (
              <button key={d} onClick={() => setSelectedDifficulty(d)}
                ...
                style={{
                  ...
                  color: selectedDifficulty === d ? (d === 'All' ? '#ffa116' : diffColor(d)) : '#64748b',
            ...
            <div className="relative">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="2.5" className="absolute left-3 top-1/2 -translate-y-1/2">
                <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
              </svg>
              <input type="text" placeholder="Search questions" value={search} onChange={e => setSearch(e.target.value)}
                className="pl-9 pr-4 py-2 rounded-lg text-sm input-glow"
                style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#e2e8f0', outline: 'none', width: '220px' }} />
            </div>
            <span className="text-xs" style={{ color: '#64748b' }}>{solved}/{total} Solved</span>
          </div>

          {/* Problem Table */}
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
            {/* Header */}
            <div className="grid grid-cols-12 gap-4 px-5 py-3 text-xs font-semibold" style={{ background: 'rgba(30,41,59,0.6)', color: '#64748b', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            {filtered.map((data, index) => (
              <Link to={data.name} key={index} style={{ textDecoration: 'none' }}>
                <div className="grid grid-cols-12 gap-4 px-5 py-3.5 items-center transition-all duration-200 cursor-pointer"
                  ...
                  <div className="col-span-1 text-sm" style={{ color: '#64748b' }}>
                    {index < solved ? (
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="#00b8a3"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
                    ) : <span>{index + 1}</span>}
                  </div>
                  <div className="col-span-6 text-sm font-medium truncate" style={{ color: '#e2e8f0' }}>
                    {index + 1}. {data.name}
                  </div>
                  <div className="col-span-2 text-xs" style={{ color: '#94a3b8' }}>
                  ...
                  <div className="col-span-1 text-xs truncate" style={{ color: '#64748b' }}>{data.tag}</div>
                </div>
              </Link>
            ))}
            {filtered.length === 0 && (
            ...
            <div className="sticky top-24 space-y-5">
              ...
              <div className="p-4 rounded-xl" style={{ background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-semibold" style={{ color: '#e2e8f0' }}>Day 5</span>
                  <span className="text-xs" style={{ color: '#64748b' }}>{total - solved} left</span>
                </div>
                {/* Calendar */}
                <div className="mb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-medium" style={{ color: '#94a3b8' }}>{monthName}</span>
                  </div>
                  <div className="grid grid-cols-7 gap-1 text-center">
                    {['S','M','T','W','T','F','S'].map((d,i) => (
                      <div key={i} className="text-xs py-1" style={{ color: '#64748b' }}>{d}</div>
                    ))}
              ...
              <div className="p-4 rounded-xl" style={{ background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-semibold" style={{ color: '#e2e8f0' }}>Weekly Premium</span>
                  <span className="text-xs" style={{ color: '#64748b' }}>2 days left</span>
                </div>
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-medium" style={{ color: '#00b8a3' }}>0 Redeem</span>
                  </div>
              ...
              {/* Progress */}
              <div className="p-4 rounded-xl" style={{ background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <p className="text-sm font-semibold mb-3" style={{ color: '#e2e8f0' }}>Progress</p>
                <div className="space-y-2">
                  {[
                    { label: 'Easy', done: 3, total: problems.filter(p=>p.difficulty==='Easy').length, color: '#00b8a3' },
                    { label: 'Medium', done: 2, total: problems.filter(p=>p.difficulty==='Medium').length, color: '#ffc01e' },
                    { label: 'Hard', done: 0, total: problems.filter(p=>p.difficulty==='Hard').length, color: '#ff375f' },
                  ].map((s,i) => (
                    <div key={i}>
                      <div className="flex justify-between text-xs mb-1">
                        <span style={{ color: s.color }}>{s.label}</span>
                        <span style={{ color: '#64748b' }}>{s.done}/{s.total}</span>
                      </div>
